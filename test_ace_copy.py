...
if __name__ == '__main__':
    ...
    testset = CamLocDataset(
        scene_path / "L2_IFGSM",# "test", "test_adv", "test" To run the test "test_twentyL2advex" "L1_IFGSM" "L2_IFGSM"
        mode=1,  # Default for ACE, we don't need scene coordinates/RGB-D. change from 0 to 1 for gt_sc
    ...
    test_log_file = output_dir / f'V2testL2IFGSMadv_{scene_name}_{opt.session}.txt'
    ...
    pose_log_file = output_dir / f'V2posesL2IFGSMadv_{scene_name}_{opt.session}.txt'
    _logger.info(f"Saving per-frame poses and errors to: {pose_log_file}")

    # Store coordinates ################################
    coord_log= output_dir / f'V2_3D_coordIFGSML2advex.txt'
    ...
    with torch.no_grad():
        for image_B1HW, _, gt_pose_B44, _, intrinsics_B33, _, gt_sc, filenames in testset_loader: # added gt_sc
            ...
            frame_nameC = Path(filenames[0]).name
            ...
            L1_loss = L1_fn(gt_sc,scene_coordinates_B3HW)

            #L2 loss
            L2_fn = torch.nn.MSELoss()
            L2_loss = L2_fn(gt_sc, scene_coordinates_B3HW)

            # Losses
            losses = (L1_loss, L2_loss)
            loss.append(losses)
            ...
            # Remove path from file name
            frame_nameC = Path(filenames[0]).name
            test_coord.write(f"{frame_nameC} {scene_coordinates_B3HW}\n")
    ...
    # save losses
    torch.save(loss, "V2_L2_IFGSM.pt")
